...
function ModernizrFilter(inputTree, options, type) {
...
ModernizrFilter.prototype.findStringMatches = function (_class, file, data) {
  ...
  // If it's not JS, assume it's CSS (or similar, e.g.: LESS, SCSS) files
    ...
    prefix = config.extensibility.cssclassprefix || '';
...
ModernizrFilter.prototype.processString = function(content, relativePath) {
  console.log("content for " + relativePath, content, content.length());
